...
fnFs_all <- sort(list.files(miseq_path, pattern="_R1_001.*trimmed\\.fastq.gz$", full.names = TRUE))
fnRs_all <- sort(list.files(miseq_path, pattern="_R2_001.*trimmed\\.fastq.gz$", full.names = TRUE))
...
# Filter and trim single pair
out <- filterAndTrim(fnFs, filtF, fnRs, filtR, 
                     truncLen=c(225,160), maxN=0, maxEE=c(2,2), 
