...
miseq_path <- Sys.getenv("BASE_DIR")
...
# Find fastq files
fnFs <- sort(list.files(miseq_path, pattern="_R1_001.*trimmed\\.fastq$", full.names = TRUE))
fnRs <- sort(list.files(miseq_path, pattern="_R2_001.*trimmed\\.fastq$", full.names = TRUE))
