...
filtF <- file.path(filt_path, paste0(sampleName, "_F_trimmed.fastq.gz"))
filtR <- file.path(filt_path, paste0(sampleName, "_R_trimmed.fastq.gz"))

# Filter and trim pairs
out <- filterAndTrim(fnFs, filtF, fnRs, filtR, 
                     trimLeft=c(19,20), maxN=0,
                     truncQ=0, rm.phix=FALSE, compress=TRUE, 
