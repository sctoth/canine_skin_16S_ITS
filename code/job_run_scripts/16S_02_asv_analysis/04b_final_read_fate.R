...
# ---------------------------
metadata <- read.delim(
  file.path(metadata_dir, "CH-All_16S_Metadata_20220124.tsv"),
...
output_file <- file.path(filt_reads, "raw_trim_trunc_read_count.csv")
...
# ---------------------------
# 4. Denoised paired reads
# ---------------------------
denoised_paired_reads <- fread(
  file.path(base_dir, "denoised_paired_read_count.csv")
)

if ("Sample" %in% names(denoised_paired_reads)) {
  denoised_paired_reads <- denoised_paired_reads %>% rename(sample = Sample)
}

denoised_paired_reads <- denoised_paired_reads %>% 
  mutate(
    # Clean the sample names to match the SampleID format
    #sample = stringr::str_remove(sample, "-16S"),
    #sample = stringr::str_remove_all(sample, "-"),
    denoised_total = denoised_forward + denoised_reverse
  ) %>% 
  # Look up and replace with Sample_Unique
  left_join(
    metadata %>% select(SampleID, Sample_Unique), 
    by = c("sample" = "SampleID")
  ) %>% 
  mutate(sample = Sample_Unique) %>% 
  select(-Sample_Unique) %>% 
  # Standardize using your helper function
  standardize_sample("sample")
...
# If denoised_paired_reads still has Sample
if ("Sample" %in% names(denoised_paired_reads)) {
  denoised_paired_reads <- denoised_paired_reads %>% rename(sample = Sample)
}
...
merged_final_df <- raw_and_filtered_read_count %>%
  ...
  left_join(denoised_paired_reads, by = "sample")
...
merged_final_df <- merged_final_df %>%
  ...
  relocate(
    sample, Individual,
    Raw_Sum, Trimmed_Sum, Trunc_Sum,
    denoised_forward, denoised_reverse, denoised_total,
    merged_reads, chimeras_removed_reads
  )
...
# ---------------------------
missing_samples <- merged_final_df %>%
  filter(is.na(merged_reads) | is.na(chimeras_removed_reads))

if (nrow(missing_samples) > 0) {
  message("Warning: Some samples missing merged/chimera counts")
  print(missing_samples$sample)
}