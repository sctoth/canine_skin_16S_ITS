...
# 4b) Save number of reads after denoising
cat("Calculating read statistics after denoising...\n")

# Extract total filtered/denoised reads per sample
denoised_forward_counts <- sapply(dadaFs, function(x) sum(x$denoised))
denoised_reverse_counts <- sapply(dadaRs, function(x) sum(x$denoised))

# Create a data frame to look at or export later
denoised_counts_df <- data.frame(
    Sample = names(dadaFs),
    Denoised_Forward = denoised_forward_counts,
    Denoised_Reverse = denoised_reverse_counts
)

write.table(denoised_counts_df, file.path(out_dir2, "denoised_read_counts.tsv", sep="\t", quote=FALSE, row.names=FALSE))


# 5) Merge, sequence table, chimera removal 
start_time <- Sys.time()
cat("Merging and chimera removal start time:", format(start_time, "%Y-%m-%d %H:%M:%S"), "\n")
