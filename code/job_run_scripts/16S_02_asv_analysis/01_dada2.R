...
denoisedF <- sapply(dadaFs, getUniques)  
denoisedR <- sapply(dadaRs, getUniques)
...
cat("Total forward reads (after denoising, before merging):", total_forward_denoised, "\n")
cat("Total reverse reads (after denoising, before merging):", total_reverse_denoised, "\n")
end_time <- Sys.time()
duration <- end_time - start_time
...
end_time <- Sys.time()
duration <- end_time - start_time
cat("Merging paired-end reads End time:", format(end_time, "%Y-%m-%d %H:%M:%S"), "\n")
cat("Merging paired-end reads: Total runtime:", round(as.numeric(duration, units="mins"), 1), "minutes\n")
