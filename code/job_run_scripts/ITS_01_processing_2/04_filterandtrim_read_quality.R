...
# Find fastq files
fnFs <- sort(list.files(miseq_path, pattern="_F_trimmed.fastq.gz$", full.names = TRUE))
fnRs <- sort(list.files(miseq_path, pattern="_R_trimmed.fastq.gz$", full.names = TRUE))
